    ...
    <div class="event-details">
      <div class="detail-item">
        ...
        <div class="detail-value">
          <a
            href="https://gnosisscan.io/block/{{@event.blockNumber}}"
            target="_blank"
      ...
      <div class="detail-item">
        ...
        <div class="detail-value" style="font-size: 0.85em;">
          <a
            href="https://gnosisscan.io/tx/{{@event.txHash}}"
            target="_blank"
            rel="noopener noreferrer"
            style="color: #667eea; text-decoration: none;"
          >
            {{substring @event.txHash 0 10}}...{{substring
              @event.txHash
              (subtract (length @event.txHash) 8)
              (length @event.txHash)
            }}
          </a>
        </div>
      ...
      {{#if @showChange}}
        <div class="detail-item">
          ...
          <div
            class="detail-value
              {{if (gt @event.percentageChange 0) 'percentage-increase'}}
              {{if (lt @event.percentageChange 0) 'percentage-decrease'}}"
          >
            {{if (gt @event.percentageChange 0) "↑"}}
            {{if (lt @event.percentageChange 0) "↓"}}
            {{formatNumber
              (abs @event.percentageChange)
        ...
        <div class="detail-item">
          ...
          <div
            class="detail-value
              {{if (gt @event.priceChange 0) 'percentage-increase'}}
              {{if (lt @event.priceChange 0) 'percentage-decrease'}}"
          >
            {{if (gt @event.priceChange 0) "↑"}}
            {{if (lt @event.priceChange 0) "↓"}}
            {{formatPrice (abs @event.priceChange)}}
          </div>
        </div>
      {{/if}}
    </div>
