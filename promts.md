update the cache on swarm, iff the stamp signing key and etherscan key are supplied, if not use the latest cache from swarm
if no cache and etherscan key is presented use just the etherscan key
the load the website every minute with the right key from many sources
of course the concerns could be split and an aggregator node run, but this makes the dev simple for this project
it also guarantees a user can get the latest values if they have their own key
